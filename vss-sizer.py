...
class IndexTest(object):
    ...
    def test(self) -> dict:
        ...
        pool_params = zip(keys, repeat(self.vec_dim), repeat(self.float_type))
        ...
        data_ram: float = round(tot_ram - index_ram - base_ram, 2)  # calculate the data usage alone by substracting base and index
        ratio: float = round(index_ram/data_ram * 100, 2)  # calculate the index to data memory ratio
        return {'index_ram': index_ram, 'data_ram': data_ram, 'index_data': ratio}

def load_db(keys: int, vec_dim: int, float_type: FLOAT_TYPE) -> None:
    ...
    """
    global connection
    for i in range(keys):   # loop creating JSON objects with 1 field - a vector of floats
